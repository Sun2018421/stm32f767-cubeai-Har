...
#define AI_NET_OBJ_INSTANCE g_hargithub
 
#undef AI_HARGITHUB_MODEL_SIGNATURE
#define AI_HARGITHUB_MODEL_SIGNATURE     "dc582ba86ee8a11fd06b698b258a4310"
...
#define AI_TOOLS_DATE_TIME   "Mon Oct 12 19:42:04 2020"

#undef AI_TOOLS_COMPILE_TIME
#define AI_TOOLS_COMPILE_TIME    __DATE__ " " __TIME__

#undef AI_HARGITHUB_N_BATCHES
#define AI_HARGITHUB_N_BATCHES         (1)
...
AI_NETWORK_OBJ_DECLARE(
  ...
  AI_BUFFER_OBJ_INIT(AI_BUFFER_FORMAT_U8,
                     1, 1, 24576, 1,
                     NULL),
  AI_TENSOR_LIST_IO_OBJ_INIT(AI_FLAG_NONE, AI_HARGITHUB_IN_NUM, &input_0_output),
  AI_TENSOR_LIST_IO_OBJ_INIT(AI_FLAG_NONE, AI_HARGITHUB_OUT_NUM, &dense_3_nl_output),
  &conv2d_1_layer, 0, NULL)



AI_DECLARE_STATIC
ai_bool hargithub_configure_activations(
...
AI_DECLARE_STATIC
ai_bool hargithub_configure_weights(
  ...
  ai_handle network, ai_network_report* report)
{
  ...
  {
    ai_network_report r = {
      .model_name        = AI_HARGITHUB_MODEL_NAME,
      .model_signature   = AI_HARGITHUB_MODEL_SIGNATURE,
...
AI_API_ENTRY
ai_error ai_hargithub_get_error(ai_handle network)
{
  return ai_platform_network_get_error(network);
}

AI_API_ENTRY
ai_error ai_hargithub_create(
...
AI_API_ENTRY
ai_handle ai_hargithub_destroy(ai_handle network)
{
  return ai_platform_network_destroy(network);
}

AI_API_ENTRY
ai_bool ai_hargithub_init(
  ai_handle network, const ai_network_params* params)
{
  ...
  ai_bool ok = true;
  ok &= hargithub_configure_weights(net_ctx, &params->params);
  ok &= hargithub_configure_activations(net_ctx, &params->activations);
...
AI_API_ENTRY
ai_i32 ai_hargithub_forward(ai_handle network, const ai_buffer* input)
{
